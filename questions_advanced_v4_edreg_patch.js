(() => {
  ...
  for (let idNum = 1; idNum <= 90; idNum++) {
    ...
    if (m === 3) {
      ...
      q.question = `某E-dReg得標小時內，連續${intervalCount}個15分鐘區間均為${typ}排程，各區間平均每秒功率皆為${trim(p)}MW；${typ}電能服務價格為${price}元/MWh。這${intervalCount}個區間的電能服務費合計為何？`;
      q.option_a = fmt(fee);
      q.option_b = fmt(price * p);
      q.option_c = fmt(fee * 2);
      q.option_d = fmt(fee / 2);
      q.answer = 'A';
      ...
      q.question = `某E-dReg得標小時內，連續${intervalCount}個15分鐘區間均為${typ}排程，且各區間平均功率相同；${typ}電能服務價格為${price}元/MWh，這些區間合計電能服務費為${fmt(fee)}。反推每區間平均每秒功率為何？`;
      q.option_a = `${trim(p)}MW`;
      q.option_b = `${trim(p * 2)}MW`;
      q.option_c = `${trim(p / 2)}MW`;
      q.option_d = `${trim(p + 1)}MW`;
      q.answer = 'A';
  ...
  fullCases.forEach(c => {
    ...
    q.question = `某E-dReg得標小時：容量結清價${c.cp}元/MW·h、效能價475元/MW·h、得標${c.mw}MW、服務品質指標${c.q}；4個15分鐘區間為${schedule}。暫不扣月結算電能損失費，該小時結算價金為何？`;
    ...
    if (q.option_b === q.option_a) {
      let wrongEnergy = 0;
      c.intervals.forEach(([state,p]) => {
        if (state === '放電') wrongEnergy += 2000 * p;
        if (state === '充電') wrongEnergy += 500 * p;
      });
